<?php
/**
* Theme functions used by SoundStage
*
...
*/ 
	$themename = 'soundstage';
	$themenamefull = 'SoundStage';
	$themeslug = 're';
	$pagedocs = 'http://cyberchimps.com/question/using-the-soundstage-page-options/';
	$sliderdocs = 'http://cyberchimps.com/question/how-to-use-the-soundstage-slider/';
	$root = get_template_directory_uri(); 
	
/**
* Basic theme setup.
*/ 
function soundstage_theme_setup() {
...
add_action( 'after_setup_theme', 'soundstage_theme_setup' );

/** Replaces RSS link from HTML 'head' with custom feed also used with the 'social icon' */
function soundstage_feed_links() {
...
add_action('wp_head', 'soundstage_feed_links');

/**
* Redirect user to theme options page after activation.
*/ 
if ( is_admin() && isset($_GET['activated'] ) && $pagenow =="themes.php" ) {
	wp_redirect( 'themes.php?page=soundstage' );
}

/**
* Add link to theme options in Admin bar.
*/ 
function soundstage_admin_link() {
	global $wp_admin_bar;

	$wp_admin_bar->add_menu( array( 'id' => 'soundstage', 'title' => 'soundstage Options', 'href' => admin_url('themes.php?page=soundstage')  ) ); 
}
add_action( 'admin_bar_menu', 'soundstage_admin_link', 113 );

/**
* Custom markup for gallery posts in main blog index.
*/ 
function soundstage_custom_gallery_post_format( $content ) {
...
add_filter('response_post_formats_gallery_content', 'soundstage_custom_gallery_post_format' ); 
	
/**
* Set custom post excerpt link text based on theme option.
*/ 
function soundstage_excerpt_link($more) {
...
add_filter('excerpt_more', 'soundstage_excerpt_link');

/**
* Set custom post excerpt length based on theme option.
*/ 
function soundstage_excerpt_length($length) {
...
add_filter('excerpt_length', 'soundstage_excerpt_length');

/**
* Custom featured image size based on theme options.
*/ 
function soundstage_featured_image() {	
...
add_action( 'init', 'soundstage_featured_image', 11);	

/**
* Add Google Analytics support based on theme option.
*/ 
function soundstage_google_analytics() {
	global $themename, $themeslug, $options;
	
	echo stripslashes ($options->get($themeslug.'_ga_code'));

}
add_action('wp_head', 'soundstage_google_analytics');

/**
* Add custom header scripts support based on theme option.
*/ 
function soundstage_custom_scripts() {
	global $themename, $themeslug, $options;
	
	echo stripslashes ($options->get($themeslug.'_custom_header_scripts'));

}
add_action('wp_head', 'soundstage_custom_scripts');

	
/**
* Register custom menus for header, footer.
*/ 
function soundstage_register_menus() {
	register_nav_menus(
	array( 'header-menu' => __( 'Header Menu' ))
  );
}
add_action( 'init', 'soundstage_register_menus' );
	
/**
* Menu fallback if custom menu not used.
*/ 
function soundstage_menu_fallback() {
	global $post; ?>
...
* Register widgets.
*/ 
function soundstage_widgets_init() {
...
add_action ('widgets_init', 'soundstage_widgets_init');

/**
* Initialize soundstage Core Framework.
*/ 
